...
class UserDashboard {
  constructor() {
    // Use the database instance from the database.js file
    this.db = database;
  }
  ...
  async getDailyStats(userId, date) {
    ...
    let rows = await this.db.execute(query, [userId, date]);
    ...
    if (rows.length === 0) {
      ...
      rows = await this.db.execute(query, [date, userId, date]);

      // Get new referrals separately
      const referralRows = await this.db.execute(
  ...
  async getInvestmentStats(userId) {
    ...
    if (stats.total_investments > 0) {
      ...
      planRows = await this.db.execute(planQuery, [userId]);
    }
  ...
  async getTeamStats(userId) {
    ...
    if (stats.total_team_size > 0) {
      ...
      levelRows = await this.db.execute(levelQuery, [userId, userId]);
      ...
      topPerformers = await this.db.execute(topPerformersQuery, [
        userId,
        userId,
      ]);
    }
  ...
  async getSystemStats(date = null) {
    ...
    const rows = await this.db.execute(query, [targetDate]);
    return rows[0];
  }
