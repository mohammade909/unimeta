...
class Notification {
  ...
  static async getByUserId(userId, page = 1, limit = 20) {
    ...
    try {
      const dbResult = await db.execute(query, [userIdNum]);
  ...
  static async markAsRead(notificationId, userId) {
    ...
    try {
      const dbResult = await db.execute(query, [notificationIdNum, userIdNum]);
      const result = Array.isArray(dbResult) ? dbResult[0] : dbResult;
  ...
  static async delete(notificationId) {
    ...
    try {
      const dbResult = await db.execute(query, [notificationId]);
      const result = Array.isArray(dbResult) ? dbResult[0] : dbResult;
      return result.affectedRows > 0;
    } catch (error) {
  ...
  static async getWithFilters(userId, filters = {}, page = 1, limit = 20) {
    ...
    try {
      const dbResult = await db.execute(query, queryParams);
      const notifications = Array.isArray(dbResult) ? dbResult[0] : dbResult;
      return Array.isArray(notifications) ? notifications : [];
    } catch (error) {
  ...
  static async deleteOldNotifications(daysOld = 30) {
    ...
    try {
      const dbResult = await db.execute(query, [daysOldNum]);
      const result = Array.isArray(dbResult) ? dbResult[0] : dbResult;
      return result.affectedRows || 0;
    } catch (error) {
