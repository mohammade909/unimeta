...
class User {
  ...
  static async autoAssignRewards(userId) {
    try {
      ...
      // Use immediate async call instead of setTimeout for better reliability
      setImmediate(async () => {
        try {
  ...
  static async verifyEmail(id) {
    const sql = 'UPDATE users SET email_verified_at = NOW(), status = "active", updated_at = NOW() WHERE id = ?';
    await db.query(sql, [id]);
  }

  // Verify phone
  static async verifyPhone(id) {
    const sql = "UPDATE users SET phone_verified_at = NOW(), updated_at = NOW() WHERE id = ?";
    await db.query(sql, [id]);
  }
    ...
    try {
      ...
      const sql = `
        SELECT 
          ...
          COALESCE(w.roi_balance, 0) as roi_balance,
          COALESCE(w.commission_balance, 0) as commission_balance,
          COALESCE(w.bonus_balance, 0) as bonus_balance,
          COALESCE(w.locked_amount, 0) as locked_amount,
          COALESCE(w.withdrawal_limit, 0) as withdrawal_limit,
  ...
  static async getReferrals(userId, options = {}) {
    ...
    try {
      // Get total count
      const countSql = "SELECT COUNT(*) as total FROM users WHERE referrer_id = ?";
      ...
      const totalEarnedSql = `
        SELECT COALESCE(SUM(w.total_earned), 0) as total_referrals_earned
        FROM users u
        LEFT JOIN user_wallets w ON u.id = w.user_id
        WHERE u.referrer_id = ?
      `;
      ...
      const totalReferralsEarned = totalEarnedResult[0].total_referrals_earned || 0;
  ...
  static async getStats() {
    try {
      ...
      console.error("Database error in User.getStats:", error);
      throw error;
    }
  }
  ...
  static async softDelete(id) {
    const sql = 'UPDATE users SET status = "banned", updated_at = NOW() WHERE id = ?';
    await db.query(sql, [id]);
  }
  ...
  static async search(query, limit = 10) {
    ...
    const params = [
      ...
      exactMatch, exactMatch, startsWith, startsWith,
      limit,
    ];
